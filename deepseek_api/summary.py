...
chat_summary_prompt = """请你根据以下对话，提取关键信息，给出总结。应使用简洁的语言，尽量包含较多的内容，输出不能多于100字。如果某一条对话为json格式，你需要视json中的\"role_response\"项的内容为这条对话的内容。"""

day_summary_prompt = """请根据以下多条包含时间的对话总结，提取关键信息，生成一份简洁的总对话总结。注意开头为多条总结的时间(年月日)，总结中应使用较粗略的时间词描述对话发生的时间段，使用简洁干练的语言，避免冗长描述。
例如以下输入：
...
你应该给出如下总结：
2024年9月7日。上午9点左右，{user}分享了最近的生活经历和观看的电影，晴表现出兴趣并参与讨论。在上午11点左右，{user}在编程中遇到问题，晴主动提供帮助，建议了有效的提示词，并表现出对{user}的关注和支持。"""
